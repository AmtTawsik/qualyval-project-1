...
const totalDoc = () => {
    collection?.find({ email: { $ne: "admin@gmail.com" } })
        .then(data => {
...
const paginateUsers = () => {
    paginatedUsers.value = users.value
}
    ...
    <NuxtLayout name="header">
        <section class="w-11/12 mx-auto mt-5">
            <div class="mb-10">
                ...
                <div v-if="!loading" class="flex justify-between items-center mb-10">
            ...
            <div v-if="!loading" class="flex justify-center items-center mt-8">
                <nav class="flex" aria-label="Pagination">
                    <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1"
                        class="border border-gray-300 text-gray-600 hover:bg-gray-200 px-3 py-1 rounded-l-md">
                        Previous
                    </button>
                    <template v-if="totalPages > 1">
                        <button v-if="currentPage > 2" @click="changePage(1)"
                            class="px-3 py-1 border-t border-b border-gray-300 text-gray-600 hover:bg-gray-200">
                            1
                        </button>
                        <span v-if="currentPage > 3" class="px-3 py-1 border-t border-b border-gray-300 text-gray-600 hover:bg-gray-200">...</span>
                        <template v-for="page in Math.min(totalPages - Math.max(currentPage - 2, 0), 3)" :key="currentPage + page - 2">
                            <button @click="changePage(currentPage + page - 2)"  :class="{
                                'bg-blue-500 text-white': currentPage === currentPage + page - 2,
                                'border border-gray-300 text-gray-600 hover:bg-gray-200': currentPage !== currentPage + page - 2
                            }" class="px-3 py-1 border-t border-b border-gray-300 text-gray-600 hover:bg-gray-200">
                                {{ currentPage + page - 2 }}
                            </button>
                        </template>
                        <span v-if="currentPage < totalPages - 2" class="px-3 py-1 border-t border-b border-gray-300 text-gray-600 hover:bg-gray-200">...</span>
                    ...
                    <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages"
                        class="border border-gray-300 text-gray-600 hover:bg-gray-200 px-3 py-1 rounded-r-md">
                        Next
                    </button>
                </nav>
            </div>
